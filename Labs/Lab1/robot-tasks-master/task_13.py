...
def explore_corridor():
    """ ротбот бежит до конца корридора, измеряет длину и ищет вход и
...
def exit_to_down():
    ...
    if exit_to_down_cell == 0:
        ...
        if from_up_enter_cell == 0:  # на случай когда наверх выхода
                                     # нет
            for i in range(lenth_of_corridor - exit_to_down_cell):
                move_left()
            move_down()
        elif from_up_enter_cell > exit_to_down_cell:
            for i in range(from_up_enter_cell - exit_to_down_cell):
                move_left()
            move_down()
        else:
            for i in range(exit_to_down_cell - from_up_enter_cell):
                move_right()
            move_down()
        return True
